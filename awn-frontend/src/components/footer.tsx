...
export default function FooterSection({ locale = "ar" }: { locale?: Locale }) {
  const base = `/${locale}`;
  const isAr = locale === "ar";

  // Minimal, relevant links for Awn
  const links = isAr
    ? [
        { title: "المزايا", href: `${base}#features` },
        { title: "المختصّون", href: `${base}/therapists` },
        { title: "الأسعار", href: `${base}/pricing` }, // point where you’ll add pricing later
        { title: "الأسئلة الشائعة", href: `${base}/faq` }, // FAQ (replacing Help)
    ...
    : [
        ...
        { title: "FAQ", href: `${base}/faq` }, // FAQ (replacing Help)
        { title: "About", href: `${base}/about` },
        { title: "Contact", href: `${base}/contact` },
      ];

  return (
    <footer className="py-16 md:py-24" dir={isAr ? "rtl" : "ltr"}>
      <div className="mx-auto max-w-5xl px-6">
        {/* Brand */}
        <Link href={base} aria-label="go home" className="mx-auto block size-fit">
          <Logo />
        </Link>
        ...
        <div className="my-8 flex flex-wrap justify-center gap-6 text-sm">
          {links.map((link, i) => (
            <Link
              key={i}
              href={link.href}
              className="text-muted-foreground block duration-150 hover:text-[#013D5B]"
            >
              {link.title}
            </Link>
          ))}
        ...
        {/* Social (kept minimal + consistent hover) */}
        <div className="my-6 flex flex-wrap justify-center gap-6 text-sm">
          <Link
            ...
            className="text-muted-foreground duration-150 hover:text-[#013D5B]"
          >
